...
void scoreCal(long long grid, int &p1, int &p2){
...
int playerScore(long long grid, int p) {//return score of p1 or p2  
	...
	if (isFilled(grid, 3) && isFilled(grid, 4) && isFilled(grid, 8))//3 3£¬4£¬8
		score += 1;
	if (isFilled(grid, 5) && isFilled(grid, 6) && isFilled(grid, 9))//4 5£¬9£¬6
		score += 1;
...
void updateGrid(long long &grid, int pos, int p) {
	...
	if (p >= 100){ //this is p1 turn while p=p2+100
		p = p - 100;//p=p2
		p1 = playerScore(grid, p); //we get new p1 p=p2
	}
...
int main() {
	...
	long long grid = 0;
	int p1, p2, p = 0, pos, i = 1;
	scoreCal(grid, p1, p2);
	displayGrid(grid);
	while (gameState(grid) == 3){
	if (i % 2 == 1){
	scoreCal(grid, p1, p2);
	cout << "Player 1, make your move <1-16>: ";
	cin >> pos;
	while (pos <= 0 || isFilled(grid, pos)||pos>=17){
	...
	if (i % 2 == 0){
	scoreCal(grid, p1, p2);
	cout << "Player 2, make your move <1-16>: ";
	cin >> pos;
	while (pos <= 0 || isFilled(grid, pos) || pos >= 17){
	cout << "Invalid move! Try angin." << endl;
	cout << "Player 2, make your move <1-16>: ";
...
cout << a[i] << endl;
	...
	/*int a[10] = { 10, 6, 19, 22, 32,31,44, 45,49,50};
	int n = 10;
	...
	for (int i = h+1; i<n; i++){ cout << a[i] << endl; }*/
	return 0;
}